...
if __name__ == "__main__":
    ...
    catwise_sdf = spark.sql(
        f"""
        SELECT DISTINCT
            {', '.join(catwise_df.columns[2:])}, ST_Point(ra_point, dec_point) as geom, explode(ST_S2CellIDs(ST_Point(ra_point, dec_point), 9)) AS cellId
        FROM
            table
        """
    )
    catwise_sdf.createOrReplaceTempView("table")

    catwise_sdf = spark.sql(
        f"""
        SELECT DISTINCT
            {', '.join(catwise_sdf.columns)}, explode(ST_S2CellIDs(geom, 4)) AS partitionId
        FROM
            table
        """
    )
    ...
    catwise_sdf.write.partitionBy("partitionId").mode("overwrite").parquet(
        QUERY_DATA_PATH + f"catwise/catwise_hybrid_equi_join.parquet"
    )
    log.info("Table with s2 indexes created and saved succesfully")
